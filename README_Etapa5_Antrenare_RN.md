...
```
Analizând graficul de predicție (docs/grafic_predictie.png), modelul tinde să aibă erori mai mari în punctele de extrem local (maximele zilei și minimele nopții) unde zgomotul aleatoriu (random noise) adăugat în generare este mai pronunțat. De asemenea, tranzițiile bruște simulate de vânt puternic pot introduce erori.
```
...
```
Factorul principal este zgomotul Gaussian (np.random.normal) introdus intenționat în datele de antrenare. În realitate, acest zgomot corespunde senzorilor imperfecți . Modelul încearcă să generalizeze curba sinusoidală și ignoră parțial zgomotul, ceea ce e un lucru bun , dar duce la mici diferențe față de valoarea "reală" (care conține zgomotul).
```
...
```
Eroare acceptabilă: O abatere de ±1°C este acceptabilă pentru planificarea irigațiilor.

Eroare critică: Dacă modelul prezice +2°C, dar în realitate sunt -1°C (îngheț), culturile pot fi compromise.

Prioritate: Minimizarea erorilor de tip "False Positive" pentru temperaturi pozitive iarna (să nu spunem că e cald când vine înghețul).
```
...
```
Măsuri corective:
Adăugare feature "Tendință": Includerea temperaturii din ora anterioară (t-1) ca input explicit, nu doar ora curentă.

Creșterea setului de date: Generarea a 3-5 ani de date (în loc de 30 de zile) pentru a captura și variațiile sezoniere (iarnă vs vară).
```
